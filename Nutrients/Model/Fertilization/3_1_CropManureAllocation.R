...
allocate_crop_animalType_specification = function(nutrient, param, manure_type) {
  
  if (manure_type == 'Total') {
    
    man_alloc = allocate_crop_animalType_totalManure(nutrient, param)
  }
  else {
    
    man_alloc = allocate_crop_animalType_manureType(nutrient, manure_type, param)
  }
...
general_func_nutrientDemand_manure_application = function(gross_man_alloc_df, nutrient_req_df) {
  # general function to compute the operations regarding nutrient demand after manure (solid/slurr) application
  #*   # computes two things:
  ...
  yrs = paste0('X', seq(1987,2017))
  nutrient_req_df[, yrs] = sapply(yrs, function(x) round(nutrient_req_df[, x] - gross_man_alloc_df[,x], 1))
 
  # remaining nutrient demand
  remaining_nutrient_req = nutrient_req_df
  remaining_nutrient_req = compute_manure_surplus(remaining_nutrient_req)[[1]]
  
  # manure surplus
  man_surplus = nutrient_req_df
  man_surplus = compute_manure_surplus(man_surplus)[[2]]
  
  return(list(remaining_nutrient = remaining_nutrient_req, man_surplus = man_surplus))
  rm(list=c('nutrient_req','yrs')) 
}


calc1_nutrientDemand_slurry_application = function(nutrient, main_param, param, manure_type = 'Slurry') {
  # RULE: first slurry is always applied
  # unit: kg nutrient yr-1
  
  gross_man_alloc = allocate_crop_animalType_specification(nutrient, param, manure_type)
  ...
  # remaining nutrient demand
  remaining_nutrient_req = general_func_nutrientDemand_manure_application(gross_man_alloc, nutrient_req)[[1]]

  # manure surplus
  man_surplus = general_func_nutrientDemand_manure_application(gross_man_alloc, nutrient_req)[[2]]

  return(list(remaining_nutrient = remaining_nutrient_req, man_surplus = man_surplus))
  rm(list=c('nutrient_req','yrs'))
}

calc2_nutrientDemand_solid_application = function(nutrient, main_param, param, manure_type = 'Solid') {
  # RULE: ONLY AFTER SOLID MANURE CAN BE APPLIED
  # unit: kg nutrient yr-1
  
  gross_man_alloc = allocate_crop_animalType_specification(nutrient, param, manure_type)
  nutrient_req = calc1_nutrientDemand_slurry_application(nutrient, main_param, param, 'Slurry')[[1]]
...
get_nutrientDemand_after_manureTypeApplication = function(nutrient, main_param, param, manure_type) {
  
  if (manure_type == 'Slurry') {
    
    nut_demand = calc1_nutrientDemand_slurry_application(nutrient, main_param, param, manure_type)[[1]]
  }
  else {
    nut_demand = calc2_nutrientDemand_solid_application(nutrient, main_param, param, manure_type)[[1]]
  }
...
compute_municipality_manure_surplus_animalType = function(animal_type, nutrient, manure_type) {
  # computes the total manure surplus of a given manure type following its crop application
  # unit: kg nutrient yr-1
  ...
  for (i in 1:nrow(crops_animalType)) {
    ...
    if (manure_type == 'Slurry') {
      ...
      crop_man_surplus = calc1_nutrientDemand_slurry_application(nutrient, main_param, param, manure_type)[[2]]
    }
    else {
      
      crop_man_surplus = calc2_nutrientDemand_solid_application(nutrient, main_param, param, manure_type)[[2]]
    }
...
compute_crop_manure_application_rate = function(nutrient, main_param, param, manure_type, nutrient_remaining_df) {
  ...
  if (missing(nutrient_remaining_df)==TRUE) {
    ...
    rem_nutrient_req = get_nutrientDemand_after_manureTypeApplication(nutrient, main_param, param, manure_type)
  }
  else {
  ...
  if (manure_type == 'Slurry') {
    ...
    # for solid manure, the crop N demand is after slurry application
    nutrient_req = calc1_nutrientDemand_slurry_application(nutrient, main_param, param, 'Slurry')[[1]]
  }
...
update_fodder_crop_nutrientReq_manureSurplus = function(man_surplus_df, nutrient, main_param, param, manure_type) {
  # * NOT IMPLEMENTED YET
  ...
  if (manure_type == 'Slurry') {
    ...
    crop_nut_demand = calc1_nutrientDemand_slurry_application(nutrient, main_param, param, manure_type)[[1]]
  }
  else {
    
    crop_nut_demand = calc2_nutrientDemand_solid_application(nutrient, main_param, param, manure_type)[[1]]
  }
  ...
  for (i in 1:nrow(ruminant_man_surplus)) {
    ...
    for (j in 4:ncol(ruminant_man_surplus)) {
      ...
      else if (crop_nut_demand[i,j] > ruminant_man_surplus[i,j]) {
        
        crop_nut_demand[i,j] = round(crop_nut_demand[i,j] - ruminant_man_surplus[i,j], 1)
        ruminant_man_surplus[i,j] = 0
      }
      else if (crop_nut_demand[i,j] == ruminant_man_surplus[i,j]) {
...
## HERE !!!!!!!!!!!!!!!!!!!1 ----------------------------
#*/ PROPERLY IMPLEMENT MANURE SURPLUS REDISTRIBUTION WITH UPDATED CROP NUT DEMANDS
#* // FOLLOW MANURE SURPLUS

loop_fodder_crops_manureType_application_rate = function(nutrient, manure_type, manure_surplus_fills_nutDemand) {
  ...
  # uses crop hierarchy
  ruminant_man_surplus =  compute_municipality_manure_surplus_animalType('ruminants', nutrient, manure_type)
  ...
  #  updated_crop_nut_demand = update_fodder_crop_nutrientReq_manureSurplus(man_surplus_df = ruminant_man_surplus, nutrient, main_param, param, manure_type)[[1]]
    ruminant_man_surplus = update_fodder_crop_nutrientReq_manureSurplus(man_surplus_df = ruminant_man_surplus, nutrient, main_param, param, manure_type)[[2]]
    
    # MODULE TO IMPLEMENT IF MANURE SURPLUS IS USED TO FURTHER FULFILL NUTRIENT DEMAND ------------------------------------------
    if (manure_surplus_fills_nutDemand == TRUE) {
      
      crop_man_app = compute_crop_manure_application_rate(nutrient, main_param, param, manure_type, updated_crop_nut_demand)
    }
    else {
      
      crop_man_app = compute_crop_manure_application_rate(nutrient, main_param, param, manure_type)
    }
...
loop_nonfodder_crops__manure_application_rate = function(nutrient, manure_type, manure_surplus_fills_nutDemand) {
...
loop_manure_application_rates = function(nutrient, manure_surplus_fills_nutDemand) {
  
  man_type = c('Solid','Slurry')
  sapply(man_type, function(x) loop_fodder_crops_manureType_application_rate(nutrient, x, manure_surplus_fills_nutDemand))
  sapply(man_type, function(x) loop_nonfodder_crops__manure_application_rate(nutrient, x, manure_surplus_fills_nutDemand))
}
...
compute_total_crop_manure_app_rate = function(nutrient, manure_surplus_fills_nutDemand = FALSE) {
  ...
  for (i in 1:nrow(standard_params)) {
    ...
    if (param == 'Tomato' | main_param == 'Horticulture' | param == 'Extensive_pasture') {
      next('Breaking')
    }
    else {
      ...
      crop_man_slurry = get_activity_data(module = 'Nutrients', mainfolder = 'Output', folder = 'Fertilisation', subfolder = nutrient, subfolderX2 = 'Manure_application_rates', subfolderX3 = 'Method 1', subfolderX4 = folder_div, subfolderX5 = 'Slurry', subfolderX6 = main_param, pattern = param)
      
      # calculate total manure app rate
      yrs = paste0('X', seq(1987,2017))
      crop_man_slurry[, yrs] = sapply(yrs, function(x) round (crop_man_slurry[,x] + crop_man_solid[,x], 1))
      export_file(module = 'Nutrients', 
                  file = crop_man_slurry, 
                  filename = param, 
                  folder = 'Fertilisation', 
                  subfolder = nutrient, 
                  subfolderX2 = 'Manure_application_rates',
