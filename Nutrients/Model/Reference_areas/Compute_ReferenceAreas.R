...
compute_municipality_annual_arableLand <- function(year) {
  # computes annual UAA for the specified year
  ...
  muni_store[, 'arableLand'] <- rowSums(muni_store[, -c(1,2,3)])
...
loop_municipality_AaableLand <- function() {
  ...
  for (i in seq_along(yrs)) {
    print(paste0('Calculating municipality_arableLand for ', yrs[i]))
    uaa <- compute_municipality_annual_arableLand(yrs[i])
    muni_store[, paste0('X',yrs[i])] <- uaa[, 'arableLand']
    
  }
  export_file(module = 'Nutrients', folder = 'Output', file = muni_store, filename = 'Arable_land', subfolder = 'Reference_areas', subfolderX2 = 'Arable_land')
}
