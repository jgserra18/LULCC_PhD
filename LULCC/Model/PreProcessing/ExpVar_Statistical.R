...
get_statistical_data <- function(year, pattern, subfolder) {
  ...
  stat_data <- get_activity_data('Statistical_data', pattern, subfolder)
...
spatially_explicit_statistical_data <- function(year) {
  ...
  statistical_files <- gsub(pattern = '.csv',
                            replacement = '', 
                            x = list.files(get_folderpath(main_folder = 'Activity_data', folder = 'Statistical_data')))
  ...
  for (i in statistical_files) {
    ...
    r_stat <- fasterize_admin_statistical(admin_res = as.character(admin), file = r_stat, name_id = 'Admin_id', 
                                          name_field = as.character(year), spatial_res = 100)
    r_stat <- resample_to_CLC(r_stat, TRUE)
    export_file(file = r_stat, folder = 'Exploratory_variables', filename = paste0(i, '_', year), subfolder = 'Statistical_data', subfolderX2 = 'Native')
  }
}
